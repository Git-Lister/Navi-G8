...
async def register(user_in: schemas.user.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(models.user.User).where(models.user.User.username == user_in.username))
    existing = result.scalar_one_or_none()
    if existing:
        raise HTTPException(status_code=400, detail="Username already registered")
    
    hashed = get_password_hash(user_in.password)
    db_user = models.user.User(username=user_in.username, password_hash=hashed)
    db.add(db_user)
    await db.commit()
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(models.user.User).where(models.user.User.username == form_data.username))
